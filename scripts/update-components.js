...
const isSourceProject =
    !isInNodeModules && (__dirname.includes('core-maugli-blog') || process.cwd().includes('core-maugli-blog') || __dirname.includes('core-maugli'));
...
async function walk(dir, visitor) {
    ...
    for (const entry of entries) {
        ...
        await visitor(p, entry.isDirectory());
        if (entry.isDirectory()) {
            // Не сканируем зависимости пользователя
            if (dir === userRoot && entry.name === 'node_modules') continue;
            await walk(p, visitor);
        }
    }
}

function isInsideForcePaths(p) {
    const rel = path.relative(userRoot, p);
    return FORCE_UPDATE_PATHS.some((base) => rel === base || rel.startsWith(`${base}${path.sep}`));
}

async function cleanup() {
    const removed = [];

    // Удаляем вложенные node_modules/core-maugli
    await walk(userRoot, async (p, dir) => {
        if (!dir) return;
        if (p.endsWith(path.join('node_modules', 'core-maugli')) && p !== path.join(userRoot, 'node_modules', 'core-maugli')) {
            ...
            if (userPackageData.scripts) {
                ...
                if (currentBuildScript && !currentBuildScript.includes('generate-previews.js')) {
                    ...
                    if (currentBuildScript.includes('astro build')) {
                        userPackageData.scripts.build = currentBuildScript.replace('astro build', 'node scripts/generate-previews.js && astro build');
                        updated = true;
...
async function updateComponents() {
    ...
    // Cleanup old artifacts before copying
    await cleanup();
